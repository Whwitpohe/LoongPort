...
export type AvailableGroupsByRelay = Record<number, AvailableGroupInfo[]>;
...
export function loadAvailableGroupCache(
  ...
  try {
    ...
    const restored: AvailableGroupsByRelay = {};
    for (const [relayId, groups] of Object.entries(parsed)) {
...
/**
 * 把成功刷新的中转站合并进缓存。未刷新或刷新失败的中转站必须保留原值，不能因为
 * 用户只点了一行的「刷新档位与密钥」就丢掉其它行的下拉选项。
 */
export function mergeAvailableGroupRefreshes(
  current: AvailableGroupsByRelay,
  refreshed: ReadonlyArray<readonly [number, ProvisionSummary]>,
  appId: AppId,
): AvailableGroupsByRelay {
  const next = { ...current };
  for (const [relayId, provisioned] of refreshed) {
    next[relayId] = availableGroupsForApp(provisioned, appId);
  }
  return next;
}
