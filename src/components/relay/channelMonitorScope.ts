 ...
 *
 * `/channel-monitors` 是一个中转站账号下的全量列表，而中转站页面一次只展示一个
...
export function filterChannelMonitorsByRelayForApp(
  appId: string,
  monitorsByRelay: Readonly<
  ...
  for (const [relayId, monitors] of Object.entries(monitorsByRelay)) {
    scoped[Number(relayId)] = filterChannelMonitorsForApp(appId, monitors);
  }
  return scoped;
}
