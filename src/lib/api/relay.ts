...
export const relayApi = {
  ...
  /** 实时拉对应中转站在当前 tab 可绑定的分组。 */
  listAvailableGroups: (
    relayId: number,
    app: string,
  ): Promise<AvailableGroupInfo[]> =>
    invoke("relay_list_available_groups", { relayId, app }),

  /** 获取渠道健康快照；旧站点不支持时由调用方静默保留上次成功结果。 */
  listChannelMonitors: (relayId: number): Promise<ChannelMonitorInfo[]> =>
    invoke("relay_list_channel_monitors", { relayId }),
  ...
  rebindTier: (
    ...
    invoke("relay_rebind_tier", { relayId, providerId, groupId, app }),
