...
/// 将站点返回的过期时间统一为 Unix 秒。
///
/// 旧版登录链路曾直接把前端 localStorage 的毫秒时间戳写入数据库；读取时若不归一，
/// 会把已经失效的 access token 当成数万年后才过期，从而跳过 refresh。
...
fn row_to_relay(row: &rusqlite::Row<'_>) -> rusqlite::Result<Relay> {
    Ok(Relay {
        ...
        token_expires_at: normalize_token_expires_at(row.get(9)?),
...
pub fn save_credentials(
    ...
    let token_expires_at = normalize_token_expires_at(token_expires_at);
    let AccountIdentity {
        id: account_id,
        label: account_label,
        login_identifier,
...
mod tests {
    ...
    #[test]
    fn legacy_millisecond_expiry_is_normalized_when_reading() {
        let conn = mem();
        let id = save_site(&conn, "https://a.dev", "A", "https://a.dev/v1").unwrap();
        conn.execute(
            "UPDATE loongport_relay SET auth_token = 'tok', refresh_token = 'ref', \
             token_expires_at = 1800000000000 WHERE id = ?1",
            [id],
        )
        .unwrap();
        ...
        assert!(op.token_looks_valid(1_700_000_000));
    }
